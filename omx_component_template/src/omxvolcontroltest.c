...
int main(int argc, char** argv) {
  ...
  if(argc < 2){
    ...
    while (argn_dec<argc) {
      if (*(argv[argn_dec]) =='-') {
        ...
        if (flagIsGain) {
          ...
          flagIsGain = 0;
          if(gain > 100) {
            DEBUG(DEB_LEV_SIMPLE_SEQ, "Gain should be between [0..100]\n");
            gain = 100;
          }
    ...
    DEBUG(DEB_LEV_SIMPLE_SEQ, "Input file %s", input_file);
    DEBUG(DEB_LEV_SIMPLE_SEQ, " to ");
  ...
  /** Ask the core for a handle to the volume control component
    */
  err = OMX_GetHandle(&handle, "OMX.st.audio_effect.volume", NULL /*appPriv */, &callbacks);
  if(err != OMX_ErrorNone) {
  ...
  if((gain >= 0) && (gain <100)) {
    ...
    DEBUG(DEB_LEV_SIMPLE_SEQ, "Setting Gain %d \n", gain);
